...
def main():
    ...
    for xpos in range(walls):
        holes.append(Rect(xpos * 10, 100, 10, 400)) # 넓이 10 높이 400 사각형
    game_over = False
    ...
    while True:
        ...
        if not game_over:
            ...
            if test.top <= 0 or test.bottom >= 600: # 벽에 닿았다면 기울기 반전
                slope = randint(1, 6) * (-1 if slope > 0 else 1)
                edge.inflate_ip(0, -20)
            edge.move_ip(10, slope)
            holes.append(edge)
            del holes[0]
            holes = [x.move(-10, 0) for x in holes] #전체를 -10만큼 이동하기
